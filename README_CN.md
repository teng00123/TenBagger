...
本项目是一个基于 FastAPI + Vue 3 的前后端分离量化交易平台，旨在为用户提供一个现代化、模块化的金融交易系统解决方案
。系统采用 RESTful API 架构，后端负责策略计算、数据处理与交易执行，前端提供直观的交互界面，实现了策略分析、回测、交易管理等功能
。项目采用 MIT 许可证，目前仅供学习和研究使用。
